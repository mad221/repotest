...
int	main()
{
	...
	Intern interne;
	Form *formu;
	formu = interne.makeForm("robotoy request", "make");
	formu = interne.makeForm("robotomy request", "make");
	max.signForm(*formu);
	max.executeFrom(*formu);
	delete formu;
	formu = interne.makeForm("presidential request", "cake");
	formu = interne.makeForm("presidential pardon", "cake");
	delete formu;
	formu = interne.makeForm("tree request", "make");
	formu = interne.makeForm("shrubberry creation", "make");
	max.signForm(*formu);
	max.executeFrom(*formu);
	return (0);
}